...
public:
    ...
    Car(callback cb, int a):f(cb, a) { }
    Car(Bar b, int a):br(b), f(std::bind(&Bar::doit, &b, 997), a) { }
    Car(Bar b):br(b), f(std::bind(&Bar::doit, &br, 9359), 15678) { }
...
///6
typedef std::shared_ptr<int> iptr;
class Dar {

public:

    Dar(){ 
        f = std::bind(&Dar::func, this, std::placeholders::_1);
        f1 =  std::bind(&Dar::func1, this, std::placeholders::_1);
    }
    void func(int& n) {
        std::cout<<"haha"<<"\n";
    }
    ...
    void doit(){ int n =6;f(n); }
    void doit1(){ 
        iptr ip=std::make_shared<int>();
        f1(ip); 
    }
    
private:
    std::function<void(int&)> f;
    std::function<void(iptr&)> f1;
};
...
int main()
{
    ...
    Goo g(std::bind(&Bar::doit, &b, 53), 5698);
    g.start();
    g.join();
    
    ///5拥有，has-a
    std::cout << "Car::run(): " << std::endl;
    Car c(std::bind(&Bar::doit, &b, 756), 3359);
    c.start();
    c.join();
